...
public class AnyGemCritter : GlobalNPC
{
	...
	{
		return V2Utils.NPCIDSets.GemCritters.Contains(entity.type);
	}
	...
	public override void SetDefaults(NPC npc)
	{
		...
		preyNPC.OnSwallowedBy = (PreyNPC.DelegateOnSwallowedBy)Delegate.Combine(preyNPC.OnSwallowedBy, new PreyNPC.DelegateOnSwallowedBy(OnSwallowedBy_GrantGemCritterMultiPreyGoal));
	}
		...
		if (prey2 == null || prey2.Count > 0)
		{
			foreach (PreyData prey3 in predPlayer.AsPred().StomachTracker.Prey)
			{
				if (prey3.Type == PreyType.NPC)
				{
					...
					if (distinctGemCritters.Contains(preyNPCID2))
					{
						distinctGemCrittersInTummy++;
						distinctGemCritters.Remove(preyNPCID2);
					}
				}
			}
		}
