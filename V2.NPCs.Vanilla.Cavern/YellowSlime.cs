...
public class YellowSlime : GlobalNPC
{
	public override bool InstancePerEntity => true;

	public static void V2YellowSlimeFirstFrameAI(NPC npc)
	{
		GeneralizedAIOverrides.SimpleSlimeFirstFrameAI(npc, 21);
	}

	public static bool V2YellowSlimeAI(NPC npc)
	{
	...
	public override void SetDefaultsFromNetId(NPC npc)
	{
		...
		if (npc.netID == -9)
		{
			...
			npc.AsSlime().JumpDelayBase = V2Utils.SensibleTime(0, 0, 2);
			npc.AsSlime().JumpDelayExtra = (Min: V2Utils.SensibleTime(0, 0, 0, 5), Max: V2Utils.SensibleTime(0, 0, 0, 35));
			npc.AsSlime().OccasionalHighJumps = true;
			npc.AsSlime().HighJumpFrequency = 3;
			SlimeNPC slimeNPC = npc.AsSlime();
			slimeNPC.HighJumpXModifier += 0.25f;
			SlimeNPC slimeNPC2 = npc.AsSlime();
			slimeNPC2.HighJumpYModifier += 0.4f;
	...
	public static bool CanYellowSlimeBeForceFed(NPC npc)
	{
		return true;
	}
