...
public class Dryadisque_TileEntity : ModTileEntity
{
	...
	public override void Update()
	{
		...
		else if (!((Entity)connectedNPC).active || connectedNPC.type != ModContent.ProjectileType<Dryadisque_ProjectileEntity>())
		{
			Activate();
		}
	}
	...
	public void Activate()
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (((Entity)npc).active && Utils.Distance(((Entity)npc).position / 16f, Utils.ToVector2(((TileEntity)this).Position)) < 2f && npc.type == ModContent.ProjectileType<Dryadisque_ProjectileEntity>())
			{
				connectedNPC = npc;
				return;
			}
		...
		if (Main.netMode != 1)
		{
			int num = Projectile.NewProjectile((IEntitySource)new EntitySource_TileEntity((TileEntity)(object)this, (string)null), new Vector2((float)(((TileEntity)this).Position.X * 16 + 48), (float)(((TileEntity)this).Position.Y * 16 + 32)), Vector2.Zero, ModContent.ProjectileType<Dryadisque_ProjectileEntity>(), 0, 0f, -1, 0f, 0f, 0f);
	...
	public override bool IsTileValidForEntity(int x, int y)
	{
		...
		{
			return ((Tile)(ref tile)).TileType == ModContent.TileType<Dryadisque>();
		}
		return false;
	}
