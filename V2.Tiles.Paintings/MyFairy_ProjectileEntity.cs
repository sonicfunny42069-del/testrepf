...
public class MyFairy_ProjectileEntity : ModProjectile
{
	...
	public override void SetDefaults()
	{
		...
		((Entity)((ModProjectile)this).Projectile).width = 64;
		...
		((ModProjectile)this).Projectile.AsPred().WeightGainRatio = 0.4;
		((ModProjectile)this).Projectile.AsPred().MaxStomachCapacity = 1.5;
		((ModProjectile)this).Projectile.AsPred().BaseStomachacheMeterCapacity = 425.0;
	...
	public override void AI()
	{
		...
		if (!((Tile)(ref Painting)).HasTile || ((Tile)(ref Painting)).TileType != ModContent.TileType<MyFairy>())
		{
	...
	{
		return Math.Min((int)Math.Floor(2.0 * Math.Sqrt(projectile.AsPred().ExtraWeight)), 2);
	}
